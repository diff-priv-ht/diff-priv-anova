...
# 
# d <- pplot9$data
# write_csv(d, "g1-epsfrac.csv")

#################################### plot #####################################
d <- read_csv("g1-epsfrac.csv")
...
p1 <- ggplot(d, aes(x = N, y = Power, col = as.factor(eps_frac))) +
  ...
  ggtitle(expression(paste("Effect of ", rho, " on ", G[1])))

ggsave("g1-epsfrac.png", p1, device = "png", height = 5, width = 8, units = "in", dpi = "retina")
setwd(wd)
