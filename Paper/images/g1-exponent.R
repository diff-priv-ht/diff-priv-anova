...
# 
# d <- pplot8$data
# write_csv(d, "g1-exponent.csv")

#################################### plot #####################################
d <- read_csv("g1-exponent.csv")
...
p1 <- ggplot(d, aes(x = N, y = Power, col = as.factor(exponent))) +
  ...
  labs(x = "N", col = "q") +
  ggtitle(expression(paste("Effect of q on ", G)))

ggsave("g1-exponent.png", p1, device = "png", height = 5, width = 8, units = "in", dpi = "retina")
setwd(wd)
